class SMB {
    constructor(dateId, body, type, value, link) {
        ...
        let string        = '<td>' + this.name + '</td>';
        for (let i = 1; i != dateId; i++)
            string += '<td></td>';
        this.tr.innerHTML = string + '<td class="checking"></td><td><a href="' + this.link + '" target="_blank">' + chromeI18n('open') + '</a> / <a>' + chromeI18n('delete') + '</a></td>';
        this.tr.lastElementChild.lastElementChild.addEventListener('click', (e) => {
            confirmyes.onclick = () => {
    ...
    sortRed() {
        ...
        for ( ; i != length && trs[i].children[this.dateId].className == 'red' && trs[i].children[0].innerHTML.localeCompare(this.name) < 0; i++) ;
    ...
    sortOrange() {
        ...
        for ( ; i != length && trs[i].children[this.dateId].className == 'orange' && trs[i].children[0].innerHTML.localeCompare(this.name) < 0; i++) ;
        this.body.insertBefore(this.tr, i != length ? trs[i] : null);
    }
    setName(name) {
        this.name                               = name;
        this.tr.children[0].innerHTML           = this.name;
    ...
    sortDate(name, result) {
        ...
        for ( ; i != length; i++) {
            value = trs[i].children[this.dateId].innerHTML;
            if (value == '' || !this.date.isSame(moment(value, 'LL')) || trs[i].children[0].innerHTML.localeCompare(this.name) > 0)
                break;
        }
        this.body.insertBefore(this.tr, i != length ? trs[i] : null);
    }
    sortNoDate(name) {
        ...
        for ( ; i != length && trs[i].children[0].innerHTML.localeCompare(this.name) < 0; i++) ;
        this.body.insertBefore(this.tr, i != length ? trs[i] : null);
    }
}
